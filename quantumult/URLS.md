...
#### 怎么用（capture-v3 · 打开 7 日页打卡 + 定时漏签检查）

1. 挂载「重写 + 脚本」两个资源 → 重写资源右上角更新 → MITM 勾选 `dev.coc.10086.cn`
2. **打卡本体：每天在 App 里打开一次「7日打卡分百万」页**（页面加载约 1 秒后自动打卡，无需点按钮）。脚本不打这个卡，只顺带抓 `User-Token` / 冻结载荷做诊断
3. **漏签检查（可选）**：把 `.task` 加入 QX 任务并把 `enabled` 改为 `true`，时间须落在你每天开完 App 后的 30 分钟窗口内（默认 08:40）→ 逐个有效 Token 查状态，发现「今日未签」才推送提醒；全部已签 / 凭证过期一律静默并清理死 Token
4. 清空凭证重抓：任务参数改 `DeleteCookie=true` 跑一次，再改回 `sign`

#### ⚠️ 2026-09-21 实测修正：冻结载荷是一次性的，脚本代打不成立

| 事实 | 证据 |
|------|------|
| 载荷被页面真实打卡消费一次后即失效 | 换任意账号的有效 `User-Token` 重放，服务端**能正常解密**（不报 300），但业务层统一返回 `code 10` |
| 早晨 4 个未签账号注入全部失败 | 2026-09-21 07:28–09:09，4 个号在各自 7 日页打开前注入 → 全部 `code 10`；最终 5 个号全部由人工开页完成 |

⇒ capture-v2 的「注入代打」路线（`SevenDayInjectOnOtherPage` 等）**默认关闭**，开关保留仅备将来复验。capture-v3 的定位：**抓凭证（诊断）+ 定时「漏签检查」**；能完成打卡的只有「打开 7 日页」这一个动作。

#### ⚠️ 为什么做不成定时打卡（2026-09-20 实测）
...
⇒ **做不到「打开 App 登录即签到」**：dev.coc 活动只能「开页驱动」。隔夜 cron 必然失败，故 `.task` 默认 `enabled=false`；若要漏签提醒，把 `enabled` 改为 `true` 并把时间调到开完 App 后 30 分钟内：
...
- 加密由 **WASM 模块 Wheel**（`wheel_bg.wasm`，`roto()`）完成，QX JavaScriptCore 内**无法重算** → 只能抓取页面产生的「冻结载荷」
- `gDT` 接口只下发一次性 UUID 设备令牌；`coc-aurora` = RSA 加密的 AES 密钥，`cocEnContent` = 该密钥下的 AES-GCM 密文
- **载荷是一次性的（2026-09-21 修正，早期「设备级可跨账号共用」结论作废）**：服务端接受任意账号 Token 解密，但被页面真实打卡消费后，任何重放均业务层拒绝（`code 10`）
- 7 日页打开后页面**自己会在 1 秒内自动打卡**（无需点按钮）

#### 可调开关（脚本头部）

| 变量 | 默认 | 说明 |
|------|------|------|
| `SevenDayEnable` | true | 总开关 |
| `SevenDayMaxTokens` | 20 | 最多保留的账号 Token 数 |
| `SevenDayInjectOnOtherPage` | **false** | 注入代打（实测不可行，默认关闭，保留备将来复验） |
| `SevenDayInjectOnOwnPage` | false | 在 7 日页注入（页面自己会签） |
| `SevenDayInjectTimeoutMs` | 6000 | 注入硬超时；超时立即放行响应，不拖死活动页 |
| `SevenDayTokenLabels` | `{}` | 给 Token 配可读名字，便于通知区分账号 |
| `LogDetails` | false | 输出载荷长度等诊断细节 |
...
|------|------|------|
| 0 | 打卡成功 | 通知 |
| 10 | 领取失败（业务态：今日已领取，**或冻结载荷已被消费/不可复用**） | 漏签检查判为「未签」并提醒；注入路径静默跳过代打 |
| 40 | 无权限（User-Token 30 分钟过期） | **不算漏签**：静默清理死 Token，不推送 |
| 50 | 参数不合法（缺 coc-aurora 头） | 提示重开活动页 |
| 300 | AES-GCM/RSA 解密失败（载荷格式失效） | 提示重开活动页刷新载荷 |

#### 挂载后操作

1. App 内打开任意 dev.coc 活动页 → 自动抓取 `User-Token`（每次开页刷新）与冻结载荷（仅诊断）
2. **每天开一次「7日打卡分百万」页**：页面自己完成打卡 —— 这是唯一有效的签到动作
3. 可选：启用 `.task`（`enabled=true`）做漏签检查，时间设在开完 App 后 30 分钟内
4. 清空凭证重抓：任务参数改 `DeleteCookie=true` 跑一次，再改回 `sign`
...
| 触发 | 行为 |
|------|------|
| 打开任意 dev.coc 活动页 | 从 `loginCheck` 抓新 Token 入库；页面真实打卡时顺带刷新冻结载荷（诊断用） |
| 定时任务 `argument=sign`（默认停用） | **漏签检查**：今日未签才推送提醒；已签 / 凭证过期静默并清理死 Token |
| 注入路径（`SevenDayInject*`，默认全关） | 若手动开启：未签且有载荷时重放 → 载荷一次性，实际会 `code 10` 被业务层拒绝 |

#### 日志对照（控制台常见行怎么读）

| 日志 | 含义 | 是否正常 |
|------|------|---------|
| `[7日打卡] loginCheck 下发令牌 …xxxxx（新入库） 页面=7日页/其它页` | 抓到新 Token 入库（capture-v3 仅凭证采集，不再代打） | 正常 |
| `[7日打卡] 已刷新冻结载荷, len=…`（需开 `LogDetails`） | 页面真实打卡时抓到一次性载荷 | 正常 |
| `[7日打卡] 有新令牌但无冻结载荷，跳过注入（需先开一次 7 日页抓载荷）` | 注入开关开启时才有此路径；全已签的日子抓不到载荷 | 正常 |
| `[7日打卡] xxx 载荷不可复用（code 10，非今日已领），跳过代打` | 注入重放被业务层拒绝（载荷已被页面消费） | 符合预期 |
| `[7日打卡] 无可用 Token：打开任意 dev.coc 活动页即自动抓取（过期的已自动清理）` | 死 Token 已全部清理 | 正常 |
| `==== 7日打卡 / 漏签检查 / 账号数 N / 载荷 有|无 ====` | 定时漏签检查启动 | 正常 |
| `[静默跳过] 7日打卡复核：N 个号已签，M 个过期凭证已清理，无漏签` | 全部已签 / 过期，不推送 | 正常 |
| 通知「中国移动 · 7日打卡分百万｜漏签 N 个｜…今日未打卡…」 | 确有账号未签且 Token 仍有效 | 需打开 7 日页补签 |

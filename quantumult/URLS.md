...
| `200001000001` | 网络有点拥挤，请稍后重试 | 自动重试 2 次；**次日首跑仍出现即为 payload 失效** |

### 排障决策树

- **签到报「今日已签到」+ 打卡全失败**：说明 encData 解密成功、身份识别正常（payload 未失效），
  打卡的 `200001000001` 大概率是「今日已领取」的泛化文案。**等次日定时跑验证** —— 次日尚未手动领取，
  若打卡成功即证实此判断，届时把该码补进 `DONE_CODES`
